...
def compute_metrics(eval_preds):
    ...
    return {"precision": precision, "recall": recall, "f1": f1, "accuracy": accuracy, "report":str(report)}
...
def read_corpus(dir_path):
    ...
    col_names =['Participant', 'utterance number', 'utterance length', 'word number' ,'start_time', 'end_time', 'word durnation', 'word', 'pos_tag', 
                'disflunecy_tag']
    words, labels = [],[]
    pos_tag = []
    csv_files = [f for f in os.listdir(dir_path) if f.endswith('.csv')]
    for index, file in enumerate(csv_files):
        data = pd.read_csv(os.path.join(dir_path,file),names=col_names)
        word = data['word'].tolist()
        tags = data['pos_tag'].tolist()
        label = data['disflunecy_tag'].tolist()
        words.append(word)
        labels.append(label)
        pos_tag.append(tags)
    words = flatten(words)
    labels = flatten(labels)
    pos_tag = flatten(pos_tag)
    label_set = list(OrderedDict.fromkeys(labels))
    return words, labels, pos_tag, label_set
...
if __name__ == '__main__':
    ...
    words , labels, pos_tag, label_set = read_corpus("../data/word_tag_csv_word_dur2")
    vocab = set()
    for word in words:
        vocab.add(word)
    print("number of total samples ---------->", len(words)) #21779 21779
    print("label2idx and idx2label:---------->", label2idx, idx2label)
    print("number of unique word", len(vocab))
    ...
    # make it seq to seq model utterance id and which word + pos to conceata with word timing

    training_args  = TrainingArguments(
        output_dir='../saved_models/bert_checkpoints',  # output directory
